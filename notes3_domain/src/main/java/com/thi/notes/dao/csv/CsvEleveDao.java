package com.thi.notes.dao.csv;

import static com.thi.notes.dao.csv.CsvFileHelper.readCsvFile;
...
public class CsvEleveDao implements EleveDao {
	...
	/**
	 * Methode pour récupérer la liste des élèves
	 */
	...
	private List<Eleve> dataToEleves(List<String[]> data) {
		...
		try {
			...
			for (String[] unEleve : data) {
				final String nom = unEleve[0];
				final String prenom = unEleve[1];
				final String classeStr = unEleve[2];
				final String sexeStr = unEleve[3];
				final String dateNaissanceStr = unEleve[4];
				final String adresse = unEleve[5];
